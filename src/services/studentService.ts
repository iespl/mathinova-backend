...
export class StudentService {
    ...
    static async getCourseContent(userId: string, slugOrId: string) {
    ...
    static async updateVideoProgress(userId: string, lessonId: string, videoId: string, lastWatchedPosition: number) {
        ...
        if (!video) throw new Error('Video not found in this lesson');
        ...
        let videoProgress: any = currentProgress?.videoProgress || {};
    ...
    static async recordQuizAttempt(userId: string, lessonId: string) {
        const lesson = await prisma.lesson.findUnique({
            where: { id: lessonId },
            include: { module: true, videos: true }
        });
    ...
    static async getLessonQuiz(userId: string, lessonId: string) {
        // 1. Verify Enrollment
        const lesson = await prisma.lesson.findUnique({
            where: { id: lessonId },
            include: { module: true }
    ...
    static async startQuizAttempt(userId: string, quizId: string) {
        return prisma.quizAttempt.create({
            data: { userId, quizId, startedAt: new Date() }
        });
    }

    static async submitQuizAttempt(userId: string, quizId: string, attemptId: string, answers: any[]) {
            ...
            if (userAnswer) {
                if (question.type === 'mcq_single' || question.type === 'mcq_multiple') {
                    ...
                    isCorrect = correctOptionIds.length === selectedIds.length &&
                        correctOptionIds.every((id: any) => selectedIds.includes(id));
    ...
    static async getLessonPYQs(userId: string, lessonId: string) {
        // 1. Verify Enrollment
        const lesson = await prisma.lesson.findUnique({
            where: { id: lessonId },
            include: { module: true }
    ...
    static async trackPYQView(userId: string, pyqId: string) {
        return prisma.pYQView.create({
            data: { userId, pyqId }
        });
    }

    private static async computeLessonCompletion(userId: string, lessonId: string, lesson: any) {
        ...
        // Video check
        const videoProgress = (progress.videoProgress as any) || {};
        const allVideosWatched = lesson.videos.length === 0 || lesson.videos.every((v: any) => videoProgress[v.id]?.watched === true);
