...
export class PaymentService {
     ...
     */
    static async processPaymentSuccess(orderId, gatewayRef, amount, method) {
        return prisma.$transaction(async (tx) => {
            ...
            if (order.userId) {
                // Calculate Expiry
                const validityDays = order.course.validityDays;
                let expiryDate = null;
                if (validityDays) {
                    expiryDate = new Date();
                    expiryDate.setDate(expiryDate.getDate() + validityDays);
                }
                // If validityDays is null/undefined, expiryDate is null (Lifetime)
